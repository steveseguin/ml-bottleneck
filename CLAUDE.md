...
- **Explicit expert offload is honored.** `cpuMoeLayers` (planner "Expert layers on CPU", plan-export `expertLayersOnCpu`, gold rows parsed from `--n-cpu-moe N` / `-ncmoe N` / `--cpu-moe` / `-ot exps=CPU`) pins the offloaded fraction (`getExplicitExpertOffloadFraction`), never below what memory forces. In gold projections the "peak VRAM proves a fit" shortcut only applies when the recorded peak is ≥70% of the uniform size estimate (dynamic quants run 10–30% under it); a peak far below it means the weights were not resident — experts auto-fit to the CPU by llama.cpp, or a dense spill — and the overflow physics models the run. A recorded memory size below the template's is used as the device's pool (3 GB GTX 1060, 8 GB 5060 Ti).
...
- **Backends differ from runtimes.** A framework profile may carry `backends[device.backend]` overrides — llama.cpp on Metal: 0.66× bandwidth, 6× MoE layer overhead; Intel SYCL/XPU (`backend: 'sycl'` on every Arc template): MoE overhead scale 0.25 (llama.cpp) / 0.75 (vLLM), `moePrefillScale` 0.5 on llama.cpp/Ollama (SYCL grouped-GEMM prompt processing loses ~2× on top of the dense scale; vLLM XPU needs none — its gold MoE prefill rows sit at 0.95), and on vLLM XPU small-M GEMMs at 0.10 of CUDA-class batched efficiency plus a 3× MTP draft cost (community Qwen3.8-27B GPTQ ladder: MTP gains plateau at ×1.57; the lab's 64-user sweep measures 1,039 aggregate) — and per-runtime attention/MoE overhead overrides; templates carry `kernelOverheadScale` (ROCm/Vulkan 1.5, SYCL 2, Apple M5 0.6) and `prefillEfficiencyScale` (RDNA4 0.45, 780M 0.2, V100 0.15; per-runtime maps on M5 — llama.cpp 0.8, MLX 1 — and on every Arc Pro — llama.cpp/Ollama 0.6, vLLM 1). **Arc Pro `float16` is the XMX matrix rate, not the Xe vector rate**: Intel publishes only FP32 and INT8, and Xe2 XMX runs INT8 at exactly 2× FP16, so the official ARK INT8 pins it (B70 367 → 183.5). Its `fp8` mirrors `float16` — Xe2 has no FP8 datapath. Prefill efficiency ramps with prompt length from `prefillRampFloor` and MoE prefill scales with tokens-per-expert per micro-batch.
- **Lab evidence** (`data/lab-evidence.json`, from neural.download — same author) is kept apart from the Localmaxxing gold set: rows are tagged `stock`, `lab-baseline`, or `tuned`. `tests/lab-evidence.test.mjs` requires stock/baseline rows within 0.6–1.6× of the engine, tuned rows at or below the optimized target (a target nobody reaches is not a target), and the measured shapes (Ornith depth sweep 0→8K→32K on a B70, the XPU MTP ladder) reproduced in direction and rough magnitude. Tuned rows never calibrate the stock engine. The rows do reach the UI and the SDK as *measured references*: `npm test` (via `scripts/stamp-engine.mjs`) generates `data/lab-evidence.js` from the JSON and stamps its cache key, the page passes them to `setEngineEvidence({ ...snapshot, labCases })`, `getLabEvidenceRows` expands depth sweeps / MTP ladders into one row per point, stock/baseline rows compete with gold rows for the ladder's "Nearest measured" rung (`findNearestMeasuredRun`), tuned rows get their own "Lab tuned" rung (`findLabTunedRun`: same hardware template *and* device count, closest depth), the evidence workspace lists every row against its stock projection / optimized target (`getLabValidationRows` → `calculateLabCaseProjection`), and the SDK exposes both as `result.measured.{nearest,labTuned}`. Edit the JSON, never the generated JS.
...
The plan results lead with an answer card + **ceiling ladder** (hardware ceiling → engine model → lab tuned when the neural.download lab has a tuned run for this model on this exact machine → expected real → nearest measured, each measured rung linking to its proof) so predictions are always shown against the physical ceiling (`calculateProjectionBounds`: the latency-aware roofline adds coordination *and* the irreducible runtime floor — the smallest per-layer/per-token cost any runtime has demonstrated, `calculateMinimalRuntimeOverheadSeconds` — so an overhead-bound small-active MoE gets an attainable optimized target, not its raw bandwidth number), followed by **How it scales** (`buildScalingSectionHtml`: inline SVG decode-vs-input-length, prompt-processing-vs-input-length, and throughput-vs-concurrent-users charts, each with a table alternative). The **model execution map** (`buildExecutionPlan` → `buildExecutionMapHtml`) renders the layer strip (which layers/slices/experts live on which device), strategy diagrams, and the per-token decode waterfall (`buildLayerStripHtml`, `buildDecodeWaterfallHtml`) — waterfall segments must sum to the engine's per-token total (tested).
